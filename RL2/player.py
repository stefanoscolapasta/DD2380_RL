    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                #print(max([(a, Q[s_current][a]) for a in list_pos], key=lambda x: x[1]))
                action = max([(a, Q[s_current][a]) for a in list_pos], key=lambda x: x[1])[0]
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
                ...
                #have to use nanmax because of the nan values, in fact, if we don't use nanmax, the nan values will be considered as the maximum value
            ...
            #we have to use nanmean because of the nan values, in fact, if we don't use nanmean, the nan values will be considered as the mean value
