    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                action = np.argmax([Q[s_current][a] for a in list_pos])
                Q[s_current][action] = Q[s_current][action] + self.settings.alpha * (R_total + self.settings.gamma * action - Q[s_current][action])
                # ADD YOUR CODE SNIPPET BETWEEN EX 2.1 and 2.2
